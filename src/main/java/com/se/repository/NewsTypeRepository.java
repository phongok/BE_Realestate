...
public interface NewsTypeRepository extends PagingAndSortingRepository<NewsType, Long>{
	...
	@Query(value = "SELECT * FROM realestatedb.news_type where idcategory = 1 and is_action = 1", nativeQuery = true)
	public List<NewsType> getNewsTypeSell();
	
	
	@Query(value = "SELECT * FROM realestatedb.news_type where idcategory = 2 and is_action = 1", nativeQuery = true)
